            ...
            <el-collapse-item title="集群资源" name="1">
              <el-row :gutter="20">
                <el-col :span="9" :offset="1">
                  <div ref="memBrokenLine" :style="{width: '650px', height: '400px'}"></div>
                </el-col>
                <el-col :span="9" :offset="2">
                  <div ref="cpuBrokenLine" :style="{width: '650px', height: '400px'}"></div>
                </el-col>
              </el-row>
            </el-collapse-item>
            <el-collapse-item title="集群I/O" name="2">
              <el-row :gutter="20">
                <el-col :span="9" :offset="1">
                  <div ref="dataBrokenLine" :style="{width: '650px', height: '400px'}"></div>
                </el-col>
                <el-col :span="9" :offset="2">
                  <div ref="dataDownBrokenLine" :style="{width: '650px', height: '400px'}"></div>
                </el-col>
              </el-row>
            </el-collapse-item>
          </el-collapse>
...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      if (this.$route.meta.keepAlive === false && this.$route.name == 'monitorCluster') {
        this.getInfo();
    ...
    methods: {
      ...
      queryEChartsData() {
        this.httpProxy(echarts.init(this.$refs.memBrokenLine), {
          'queryProper': {
            'queryUnitExp': 1024 * 1024 * 1024,
            'queryUnit': 'G',
            'title': '内存使用'
          },
          'queryParams': {
        ...
        this.httpProxy(echarts.init(this.$refs.cpuBrokenLine), {
          'queryProper': {
            'queryUnitExp': 1,
            'queryUnit': '%',
            'title': 'CPU使用率'
          ...
          'queryParams': {
            query: 'sum(sum(rate(container_cpu_usage_seconds_total{name=~".+"}[1m])) by (name) * 100)',
            start: new Date().getTime() / 1000 - 3600,
            end: new Date().getTime() / 1000,
            step: '14'
          }
        }),
          this.httpProxy(echarts.init(this.$refs.dataBrokenLine), {
            'queryProper': {
              'queryUnitExp': 1024,
              'queryUnit': 'M',
              'title': '网络接传输速率'
            ...
            'queryParams': {
              query: 'sum(rate(container_network_transmit_bytes_total{id="/"}[1m])) by (id)',
              start: new Date().getTime() / 1000 - 3600,
              end: new Date().getTime() / 1000,
              step: '14'
            }
          })
        this.httpProxy(echarts.init(this.$refs.dataDownBrokenLine), {
          'queryProper': {
            'queryUnitExp': 1024,
            'queryUnit': 'M',
            'title': '网络接收量速率'
          ...
          'queryParams': {
            query: 'sum(rate(container_network_receive_bytes_total{id="/"}[1m])) by (id)',
            start: new Date().getTime() / 1000 - 3600,
            end: new Date().getTime() / 1000,
            step: '14'
          }
